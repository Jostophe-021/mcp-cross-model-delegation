...
CI actions are pinned to reviewed full commit SHAs; the human version is noted in each workflow. The Docker base remains `python:3.12-slim` so rebuilt images receive upstream security fixes. This floating tag means rebuilds are not byte-for-byte reproducible. A digest pin should be added with an automated, reviewed update path; otherwise it would freeze base-image fixes. The lockfile and dependency audit cover Python packages, not the base image.
